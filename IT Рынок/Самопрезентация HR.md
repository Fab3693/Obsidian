...
4. Один яркий кейс  
    Во время крупных маркетинговых кампаний мы столкнулись с риском перерасхода бюджета: транзакции обрабатывались параллельно, и система не всегда успевала корректно контролировать лимиты. Я проанализировал проблему на уровне архитектуры и предложил техническое решение — внедрить распределённый счётчик в Redis с атомарными операциями. Он проверял остаток бюджета в реальном времени перед начислением кэшбэка. Дополнительно я реализовал механизм оповещений и fallback-сценарии, чтобы не блокировать всю обработку при сбоях. В итоге система стала надёжно удерживать лимиты без заметного влияния на скорость транзакций, а маркетинг получил стабильный инструмент для запуска акций.
